...
def make_min_discrete_action_critic(obs_dim, n_actions, hidden_dims=(128, 128), n_networks=2, activation=nn.ReLU, output_activation=None, device='auto'):
    # creates an ensemble of 2 critics and takes the min Q value
...
class SimpleCNN(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dims=(32, 64), activation=nn.ReLU, output_activation=None, device='auto', greyscale=False):
        ...
        # input_dim is (H, W, C) format
        h, w, in_channels = input_dim
        
        # Adjust channels if using greyscale
        if greyscale:
            in_channels = 1
        ...
        conv_layers = []
        for hidden_dim in hidden_dims:
            conv_layers.append(nn.Conv2d(in_channels, hidden_dim, kernel_size=3, stride=1, padding=1))
            conv_layers.append(activation())
            conv_layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
            in_channels = hidden_dim
        conv_layers.append(nn.Flatten())
        
        self.conv_model = nn.Sequential(*conv_layers).to(self.device)
        
        # Calculate flattened size
        with torch.no_grad():
            # Create random input in (H, W, C) format and preprocess to (B, C, H, W)
            rand_inp = torch.rand(h, w, input_dim[2])
            rand_inp = preprocess_obs(rand_inp, device=self.device, greyscale=self.greyscale)
            flat_size = self.conv_model(rand_inp).shape[1]
        
        # Build fully connected layers
        fc_layers = [nn.Linear(flat_size, output_dim)]
        if output_activation is not None:
            fc_layers.append(output_activation())
        
        self.fc_model = nn.Sequential(*fc_layers).to(self.device)
    
    def forward(self, x):
        x = preprocess_obs(x, device=self.device, greyscale=self.greyscale)
        x = x.to(self.device)
        x = self.conv_model(x)
        x = self.fc_model(x)
        return x

def make_cnn_sequential(input_dim, output_dim, hidden_dims=(32, 64), activation=nn.ReLU, output_activation=None, device='auto', greyscale=False):
    return SimpleCNN(input_dim, output_dim, hidden_dims, activation, output_activation, device, greyscale)


def preprocess_obs(obs, device, greyscale=False):
    ...
    if greyscale and obs.shape[1] == 3:
        # Standard RGB to greyscale conversion
        obs = (0.299 * obs[:, 0:1, :, :] + 0.587 * obs[:, 1:2, :, :] + 0.114 * obs[:, 2:3, :, :])
